  ...
  return <div className="orders-dashboard">
    ...
    <section className="order-status-panel">
      ...
      {visibleOrders.map((order) => <article className="order-card" key={order.id}>
        <div className="order-select"><input type="checkbox" checked={selectedIds.includes(order.id)} onChange={() => toggleSelection(order.id)} aria-label={`Selecionar pedido ${order.id.slice(0, 8)}`} /></div>
        <div className="lead-card-main"><div><h3>Pedido #{order.id.slice(0, 8)}</h3><p>{order.customers?.name || 'Cliente'} · {order.customers?.email || 'E-mail não informado'} · {new Date(order.created_at).toLocaleString('pt-BR')}</p><small>{order.order_items.map((item) => `${item.product_name} x${item.quantity}`).join(' · ')}</small><p className="order-payment-method">Pagamento: <strong>{order.payment_method ? paymentMethodLabel[order.payment_method] : 'Não informado'}</strong></p></div><strong>R$ {Number(order.total).toFixed(2).replace('.', ',')}</strong></div>
