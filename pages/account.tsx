...
export default function Account() {
  ...
  function update(field: keyof AccountForm, value: string) { setForm((current) => ({ ...current, [field]: field === 'document' ? formatDocument(value) : field === 'phone' ? formatPhone(value) : value })) }
  async function lookupCep(value: string) { const cep = value.replace(/\D/g, ''); update('cep', formatCep(value)); if (cep.length !== 8) { update('address', ''); update('city', ''); return } setMessage('Consultando endereço...'); try { const response = await fetch(`/api/cep?cep=${cep}`); const result = await response.json(); if (!response.ok) throw new Error(result.error); update('address', result.address.logradouro || ''); update('city', result.address.localidade ? `${result.address.localidade}/${result.address.uf || ''}` : ''); setMessage('Endereço preenchido. Informe o número e o complemento, se houver.') } catch (error) { setMessage(error instanceof Error ? error.message : 'Não foi possível consultar o CEP.') } }
