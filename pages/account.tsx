...
export default function Account() {
  const { customer, requestCode, verifyCode, signOut } = useCustomer()
  ...
  async function submit(event: FormEvent) {
    ...
    const error = codeSent ? await verifyCode(form.email, code) : await requestCode(form.email)
    setSubmitting(false)
    if (error) setMessage(error)
    else {
      setCodeSent(true)
      setMessage(codeSent ? 'Código confirmado. Conta acessível.' : 'Código enviado. Confira também a pasta de spam ou lixo eletrônico.')
    }
  }
  ...
  return <section className="container account-page"><p className="eyebrow">COMPRA SEGURA</p><h1>{mode === 'login' ? 'Entrar na sua conta' : 'Criar sua conta'}</h1><p className="account-copy">{mode === 'login' ? 'Acesse sua conta com o e-mail informado.' : 'Preencha seus dados para criar o cadastro.'}</p><button className="google-button" type="button" onClick={continueWithGoogle}>Continuar com Google</button><div className="account-divider">ou use seu e-mail</div><form className="account-form" onSubmit={submit}>{mode === 'register' && <><label>Nome completo<input required value={form.name} onChange={(event) => update('name', event.target.value)} /></label><label>CPF ou CNPJ<input required value={form.document} onChange={(event) => update('document', event.target.value)} /></label><label>Telefone<input required value={form.phone} onChange={(event) => update('phone', event.target.value)} /></label><label>CEP<input required value={form.cep} onChange={(event) => lookupCep(event.target.value)} placeholder="00000-000" /></label><label>Endereço<input required readOnly value={form.address} placeholder="Preenchido pelo CEP" /></label><label>Número<input required value={form.number} onChange={(event) => update('number', event.target.value)} /></label><label>Complemento<input value={form.complement} onChange={(event) => update('complement', event.target.value)} placeholder="Ex.: ap. 33, bloco 12" /></label><label>Cidade / UF<input required readOnly value={form.city} placeholder="Preenchida pelo CEP" /></label></>} {!codeSent ? <label className="wide">E-mail<input required type="email" value={form.email} onChange={(event) => update('email', event.target.value)} /></label> : <label className="wide">Código recebido por e-mail<input required inputMode="numeric" maxLength={6} value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, ''))} placeholder="000000" /></label>}{message && <p className="form-status">{message}</p>}<button className="primary-button" type="submit">{mode === 'register' ? 'Criar cadastro' : codeSent ? 'Confirmar código' : 'Enviar código'} <span>→</span></button></form>{mode === 'login' && codeSent && <button className="resend-button" type="button" onClick={async () => { const error = await requestCode(form.email); setMessage(error || 'Novo código enviado.') }}>Reenviar código</button>}<button className="account-switch" type="button" onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setCodeSent(false); setCode(''); setMessage('') }}>{mode === 'login' ? 'Ainda não tenho conta' : 'Já tenho uma conta'}</button></section>
}
