...
return <section className="admin-page container"><div className="admin-heading"><div><p className="eyebrow">ÁREA MASTER</p><h1>Central de atendimento</h1></div><Link href="/" className="outline-button">Voltar à loja</Link></div><AdminPushSetup /><nav className="admin-tabs"><button className={tab === 'leads' ? 'active' : ''} onClick={() => setTab('leads')}>Leads</button><button className={tab === 'orders' ? 'active' : ''} onClick={() => setTab('orders')}>Pedidos</button><button className={tab === 'products' ? 'active' : ''} onClick={() => setTab('products')}>Produtos</button><button className={tab === 'bulk' ? 'active' : ''} onClick={() => setTab('bulk')}>Cadastro</button><button className={tab === 'promotions' ? 'active' : ''} onClick={() => setTab('promotions')}>Posts promocionais</button><button className={tab === 'coupons' ? 'active' : ''} onClick={() => setTab('coupons')}>Cupons</button><button className={tab === 'questions' ? 'active' : ''} onClick={() => setTab('questions')}>Perguntas</button><button className={tab === 'store' ? 'active' : ''} onClick={() => setTab('store')}>Loja</button></nav>{tab === 'leads' && <AdminLeads onMessage={setMessage} />}{tab === 'orders' && <AdminOrders onMessage={setMessage} />}{tab === 'products' && <AdminProducts products={products} onSaved={(updated) => setProducts((items) => items.map((item) => item.id === updated.id ? updated : item))} onReordered={(ordered) => setProducts(ordered)} onMessage={setMessage} />}{tab === 'bulk' && <AdminBulkProducts onMessage={setMessage} />}{tab === 'promotions' && <AdminPromotions products={products} onMessage={setMessage} />}{tab === 'coupons' && <AdminCouponsPage />}{tab === 'questions' && <AdminQuestions onMessage={setMessage} />}{tab === 'store' && <AdminStoreProfile />}{message && <p className="form-status success">{message}</p>}</section>
}
