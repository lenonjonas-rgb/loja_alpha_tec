...
export default function Admin() {
  ...
  return <section className="admin-page container"><div className="admin-heading"><div><p className="eyebrow">ÁREA MASTER</p><h1>Central de atendimento</h1></div><Link href="/" className="outline-button">Voltar à loja</Link></div><nav className="admin-tabs"><button className={tab === 'leads' ? 'active' : ''} onClick={() => setTab('leads')}>Leads</button><button className={tab === 'orders' ? 'active' : ''} onClick={() => setTab('orders')}>Pedidos</button><button className={tab === 'products' ? 'active' : ''} onClick={() => setTab('products')}>Produtos</button><button className={tab === 'bulk' ? 'active' : ''} onClick={() => setTab('bulk')}>Cadastro</button></nav>{tab === 'leads' && <AdminLeads onMessage={setMessage} />}{tab === 'orders' && <AdminOrders onMessage={setMessage} />}{tab === 'products' && <AdminProducts products={products} onSaved={(updated) => setProducts((items) => items.map((item) => item.id === updated.id ? updated : item))} onMessage={setMessage} />}{tab === 'bulk' && <AdminBulkProducts onMessage={setMessage} />}{message && <p className="form-status success">{message}</p>}</section>
}
