...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { data: order, error: orderError } = await supabase
      .from('orders')
      .select('id,tracking_code,carrier,shipping_address,customers(name,email,phone),order_items(product_id,product_name,quantity,unit_price)')
      .eq('id', orderId)
      .maybeSingle()
    if (orderError) throw orderError
    if (!order) return res.status(404).json({ error: 'Pedido não encontrado.' })

    const destination = (order.shipping_address || {}) as Record<string, string>
    if (!destination.cep) return res.status(400).json({ error: 'Este pedido não tem endereço de entrega salvo.' })
    ...
    const { codigoObjeto } = await createPrepostagem({
      destination: {
        name: destination.name || customer?.name,
