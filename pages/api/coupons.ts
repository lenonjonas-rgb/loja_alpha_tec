...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { data, error } = await getSupabaseServer()
      .from('coupons')
      .select('id,code,discount_percent,expires_at,usage_limit,used_count,product_id')
      .eq('code', code)
      .eq('active', true)
      .maybeSingle()

    if (error && /product_id/i.test(error.message)) {
      // Fallback sem product_id se a coluna ainda não foi criada
      const fallback = await getSupabaseServer()
        .from('coupons')
        .select('id,code,discount_percent,expires_at,usage_limit,used_count')
        .eq('code', code)
        .eq('active', true)
        .maybeSingle()
      ...
      return res.status(200).json({ code: fallback.data.code, discountPercent: Number(fallback.data.discount_percent) })
    }
