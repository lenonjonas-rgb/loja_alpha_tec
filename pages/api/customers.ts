...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const { error: addressError } = await supabase.from('addresses').insert({ customer_id: user.id, cep: customer.cep || '', street: customer.address || '', number: customer.number || '', complement: customer.complement || '', city: city || '', state: state || '' })
    if (addressError) throw addressError
    return res.status(200).json({ ...data, cep: customer.cep || '', address: customer.address || '', number: customer.number || '', complement: customer.complement || '', city: customer.city || '' })
