...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  } catch (error) { console.error('Erro na API de produtos:', error); const message = error instanceof Error ? error.message : ''; const missingColumns = /stock|discount_percent|flash_sale|show_in_banner/i.test(message); return res.status(500).json({ error: missingColumns ? 'A tabela products ainda não tem os campos comerciais. Execute scripts/commerce.sql no Supabase.' : message || 'Não foi possível salvar o produto. Confira as variáveis do Supabase e a tabela products.' }) }
}
