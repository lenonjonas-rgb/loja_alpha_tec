...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    const supabase = getSupabaseServer()
    if (req.method === 'GET') { const { data, error } = await supabase.from('products').select('*').order('created_at', { ascending: false }); if (error) throw error; const migrated = await Promise.all(data.map(async (row) => { const image = row.image_url?.startsWith('data:image/') ? await persistImage(row.image_url, row.id) : row.image_url; if (image !== row.image_url) await supabase.from('products').update({ image_url: image }).eq('id', row.id); return { id: row.id, name: row.name, brand: row.brand, category: row.category, compatibleEquipment: row.compatible_equipment, description: row.description, image, price: Number(row.price), active: row.active } })); return res.status(200).json(migrated) }
